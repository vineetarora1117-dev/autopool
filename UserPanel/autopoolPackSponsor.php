...
// Query sponsor transactions for this user and package wallet
$stmt = $pdo->prepare("
    SELECT * FROM transactions 
    WHERE user_id = ? AND transaction_type = 'sponsor_income' AND wallet_type = ? 
    ORDER BY id DESC
");
            ...
            <tbody>
                <?php if (empty($incomes)): ?>
                    <tr style="height: 50px; border-bottom: 1px solid rgba(255,255,255,0.05); color:#a0aec0;">
                        <td colspan="6" style="text-align: center;">No sponsor referral income payouts recorded for this pack yet.</td>
                    </tr>
                <?php else: ?>
                    <?php 
                    $idx = 1;
