...
CREATE TABLE `user_financial_summary` (
    ...
    `net_income` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Total earned minus total withdrawn',
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `company_ledger` (
    ...
    `total_funds_received` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Cumulative USDT deposited into system',
    `unutilized_funds` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Total in all users main wallets (not spent)',
    `invested_funds` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Total spent on packages/boosters',
    `total_usdt_paid_out` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Total USDT that left system via withdrawals',
    `company_wallet_balance` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Company earnings (fees + revenue + SA000001 sweep)',
    `total_payout_liability_main` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Total sitting in all users package earning wallets',
    `total_payout_liability_booster` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Total sitting in all users booster earning wallets',
    `total_held_sponsor_income` DECIMAL(15,4) DEFAULT 0.0000 COMMENT 'Total sponsor income held pending sponsor upgrade',
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB;
